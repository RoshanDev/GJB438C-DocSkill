...
Markdown 保留 YAML front matter，使用本类型 Profile 的章节和 `gjb-*` 字段合同。按真实需求、设计、接口、数据和用例逐条展开。工程证据在其 Markdown 所在章节原位渲染为可见表格，不再默认复制整套证据到附录。把需求、设计单元、接口字段、数据库约束、测试步骤写在对应正文章节；附录仅保留补充材料，不能用它替代正文的论证、步骤、字段表和图示。缺少事实只记录待提供项，不虚构接口、表、固件、性能、版本、测试结果或签字。
...
页数由真实 Office 排版计算，只统计主文档正文章节，排除前三页、目录、前言、所有附录/附件及证据登记附篇；不能用 DOCX 页数元数据或字数估计。large 各类型门槛由 `volume-policy` 查询，项目可再提高；它们是工程策略，不是标准规定所有文件都必须若干页。禁止复制段落、空表、空白页或放大图片凑数。
...
正文复用还要求结构与资源绑定一致：正文表格结构、绘图属性、关联图片/嵌入资源，以及样式定义均纳入保守校验。仅文字相同不代表正文未改。缺少结构绑定的旧 Word 不自动复用嵌入正文或保留批准；优先使用原 Markdown 重新生成，需回流时保留原文件，人工对照候选内容和结构化证据。工具只忽略已知的目录书签/分页提示变化；其它排版变化也可能触发待审核，不保证任意 Office 重存都精确回流。

## 正文与附录（必须先检查）

`body_pages` 现在只表示主文档正文章节页数，报告 `page_count_scope` 必须是 `main_body_only_v1`；附录起点与页数单列。旧版包含附录的体量 PASS 不可复用。正文/附录之间自动独立起页；`GJB_BODY` 仍覆盖完整文档以保护回流和内容绑定。

先读 `references/main-body-policy.md`。审核现有稿件时先列正文章节与附录清单，不要再次初始化或自动删除旧稿。禁止按字符差额执行循环扩写、把字段逐行复述成“正文展开 N”、把附录改名为数字章节绕过检查，或人为调大表格/字号增加页数。真实表格和图示应位于承担该项说明职责的正文章节，附录中的同一份材料不会计入正文体量。

推荐按子系统/功能域分批完善 1–2 节并检查来源、边界、正常/异常路径和跨章追踪。每批记录真实内容缺口；不足门槛就报告不足，不把门槛当写作配额。页数足够不证明技术内容充分，仍需人工语义审阅。旧稿中的“质量门禁数据块”可暂留附录作迁移来源；迁移时将必需证据移动至所属章节并补充解释，不复制两份，不虚构达到条目数。
