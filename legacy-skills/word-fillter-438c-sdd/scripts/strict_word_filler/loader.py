...
from .template_rules import COVER_TITLE_ANCHOR, IDENTITY_LINE_ANCHORS, INSERTION_PARENT_RULES, PATTERN_OVERRIDES
...
def build_plan(config_data: dict, project_data: dict) -> BuildPlan:
    ...
    exact_rules.extend(
        [
            ExactReplaceRule("cover.project_name", "外部型号+产品名称", project_name, 1),
            ExactReplaceRule("cover.document_title", COVER_TITLE_ANCHOR, document_title, 1),
            ExactReplaceRule("cover.short_name", "产品型号-XXXX", f"产品型号-{short_name}", 1),
    ...
    for section_id, section_data in structure.items():
        ...
        if section_id in INSERTION_PARENT_RULES:
            rule_spec = INSERTION_PARENT_RULES[section_id]
            items: list[DynamicSectionItem] = []
            subsections = section_data.get("subsections", {})
            if not isinstance(subsections, dict) or not subsections:
                raise StrictDataError(f"动态章节 {section_id} 缺少 subsections")
            ...
            summary_text = _require_content({"value": section_data.get("content")}, "value")
            paragraph_rules.append(
                ParagraphReplaceRule(
                    key=f"{section_id}.summary",
        ...
        for placeholder in section_data.get("placeholders", []):
            ...
            if placeholder_id in INSERTION_PARENT_RULES:
                raise StrictDataError(
                    f"动态章节 {placeholder_id} 必须作为顶层 structure 条目，不能放在 placeholders 内"
                )
